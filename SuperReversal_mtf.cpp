...
int main()
{
    return configure("SuperReversal_mtf",
                     [](auto cfg)
                     {
        const auto fast = integer_range(3, 204, 2);
        const auto slow = integer_range(70, 590, 5);
        if (cfg.htf.empty())
        ...
        return run(
            cfg, false, warm, {{"ema_fast", fast}, {"ema_slow", slow}, {"max_open", slots(cfg)}},
            [](const Params &)
            {
            return true;
            },
            ...
            const auto f = ind.ema(p[0], true), s = ind.ema(p[1], true), st = ind.supertrend(15, 5.0f, true);
            return evaluate(
                d, w, false, p[2], ind.ready,
